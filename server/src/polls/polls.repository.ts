...
export class PollsRepository {
  ...
  async getPoll(pollID: string): Promise<Poll> {
    ...
    try {
      ...
      this.logger.error(`Failed to get poll ${pollID}`, err);
  ...
  async startPoll(pollID: string): Promise<Poll> {
    ...
    try {
      ...
      this.logger.log(`Failed to set hasStarted for poll: ${pollID}`, err);
      throw new InternalServerErrorException(
        `Failed to set hasStarted for poll: ${pollID}`,
      );
    }
  }
  ...
  async addResults(pollID: string, results: Results): Promise<Poll> {
    ...
    try {
      ...
      this.logger.log(`Failed to add results for pollID ${pollID}`, err);
      throw new InternalServerErrorException(
